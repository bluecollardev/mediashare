...
function playlistControllerCreateRaw<T>(
  ...
  if (requestTransform) {
    config.transform = (body: ResponseBody, text: ResponseBody) =>
      requestTransform(CreatePlaylistResponseDtoFromJSON(body), text);
  }
...
function playlistControllerFindOneRaw<T>(
  ...
  meta.authType = ['bearer'];
  const config: QueryConfig<T> = {
    url: `${runtime.Configuration.basePath}/api/playlists`.replace(
      `{${'playlistId'}}`,
  ...
  if (requestTransform) {
    config.transform = (body: ResponseBody, text: ResponseBody) =>
      requestTransform(PlaylistResponseDtoFromJSON(body), text);
  }
...
function playlistControllerRemoveRaw<T>(
  ...
  meta.authType = ['bearer'];
  const config: QueryConfig<T> = {
    url: `${runtime.Configuration.basePath}/api/playlists/{playlistId}`.replace(
      `{${'playlistId'}}`,
      encodeURIComponent(String(requestParameters.playlistId))
    ),
    meta,
...
function playlistControllerShareRaw<T>(
  ...
  if (requestParameters.userId === null || requestParameters.userId === undefined) {
    throw new runtime.RequiredError(
      'userId',
      'Required parameter requestParameters.userId was null or undefined when calling playlistControllerShare.'
    );
  }
