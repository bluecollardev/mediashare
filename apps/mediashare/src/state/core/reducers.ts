...
// Initial load
export const loadItems = (stateKey: string) => <T>(
...
// Update
export const updateItem = (stateKey: string) => <T>(
  state: RootState, action: PayloadAction<T>
): RootState => {
  const items = state[stateKey];
...
export const updateItems = (stateKey: string) => <T>(
  state: RootState, action: PayloadAction<T>
): RootState => {
  return { ...state }
};
...
// Remove
export const removeItem = (stateKey: string) => <T>(
  state: RootState, action: PayloadAction<T>
): RootState => {
  const items = state[stateKey];
  if (!action.payload['id']) return { ...state }; // We could throw an error here...
...
// Etc.
export const reorderItems = (stateKey: string) => <T>(
  state: RootState, action: PayloadAction<T>
) => {
  return { ...state }
};
export const resetItems = (stateKey: string) => <T>(
  state: RootState, action: PayloadAction<T>
): RootState => {
