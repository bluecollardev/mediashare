...
type EnumRecordType<S extends readonly string[]> = {
  [P in S[number] as SnakeCaseToCamelCase<P>]: P;
};

type ActionsType<S extends readonly string[], V> = {
  [P in S[number] as SnakeCaseToCamelCase<P>]: PayloadActionCreator<V, P>;
};
type ActionsTypeSc<S extends readonly string[], V> = {
...
export const makeEnum = <T extends readonly string[]>(actions: T): EnumRecordType<T> =>
  actions.reduce((prev, curr) => ({ ...prev, [snakeCaseToCamelCase(curr) as string]: curr }), Object.create({}));

export const makeActions = <T extends readonly string[], V>(actions: T): ActionsType<T, V> =>
  actions.reduce(
    (prev, curr) => ({
      ...prev,
      [snakeCaseToCamelCase(curr) as string]: createAction(curr),
    }),
    Object.create({})
  );
...
export const ActionsFactory = <T extends readonly string[], V>(actions: T, initialState: V): ActionsTypeSc<T, V> =>
  actions.reduce(
    (prev, curr) => ({
      ...prev,
      [snakeCaseToCamelCase(curr) as string]: createAction<V>(snakeCaseToCamelCase(curr) as string),
    }),
    Object.create({})
  );
