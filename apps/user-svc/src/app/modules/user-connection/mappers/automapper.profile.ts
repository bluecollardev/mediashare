...
export const connectionToConnectionDtoMappingFactory = (mapper) =>
  createMap(
    mapper,
    UserConnection,
    UserConnectionDto,
    forMember(
      (dest: UserConnectionDto) => dest._id,
      convertUsing(
    ...
    forMember(
      ...
      convertUsing(
        objectIdToStringConverter as never,
        (source: UserConnection) => source.userId
      )
    ),
    forMember(
      (dest: UserConnectionDto) => dest.connectionId,
      convertUsing(
        objectIdToStringConverter as never,
  ...
  createMap(
    ...
    forMember(
      ...
      convertUsing(
        ...
        (source: UserConnectionDto) => source.userId
      )
    ),
    forMember(
      (dest: UserConnection) => dest.connectionId,
      convertUsing(
        stringToObjectIdConverter as never,
...
export const updateConnectionDtoToConnectionMappingFactory = (mapper) =>
  createMap(
    ...
    UserConnection,
    forMember(
      (dest: UserConnection) => dest._id,
      convertUsing(
        stringToObjectIdConverter as never,
        (source: UserConnectionDto) => source._id
      )
    ),
    forMember(
      (dest: UserConnection) => dest.userId,
