...
type ApiControllerDecoratorOptions = Pick<
  ApiResponseMetadata,
  'type' | 'status' | 'isArray' | 'description'
>;

export type ApiControllerDecoratorParams =
  Partial<ApiControllerDecoratorOptions>;

const ApiGetResponse = function ({
  type,
  isArray,
...
const ApiPostResponse = function ({
  type,
  isArray,
}: ApiControllerDecoratorParams = {}) {
